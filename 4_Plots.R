...
plot(direct_obs, direct_pred_RF, xlim = c(4, 27), ylim = c(4, 27), col = rgb(0, 0,
  0, 0.1), bty = "l", main = "", cex.lab = 1.4, cex = 1.5, cex.axis = 1.3, pch = 16,
  las = 1, bty = "l", xlab = MyXlab, ylab = MyYlab)
...
mtext(bquote(paste(r^2 == .(r2), ",", " %RMSE" == .(NRMSE), "%", ",", " bias" ==
  .(bias))), side = 3, line = 0.5, adj = 0, cex = 1.3, font = 2)
legend("bottomright", legend = c("1:1 line", "fitted line"), lwd = c(2, 2), lty = c(1,
  2), cex = 1.2, bty = "n")
mtext("(a)", side = 3, line = 0.5, adj = -0.1, cex = 1.5, font = 2)

plot(direct_obs, indirect_pred_C, xlim = c(4, 27), ylim = c(4, 27), col = rgb(0,
  0, 0, 0.1), bty = "l", main = "", cex.lab = 1.4, cex = 1.5, cex.axis = 1.3, pch = 16,
  las = 1, bty = "l", xlab = MyXlab, ylab = MyYlab)
abline(0, 1, lty = 1, lwd = 2.5)
abline(lm(indirect_pred_C ~ direct_obs - 1), lty = 2, lwd = 2.5)
r2 = round((cor(indirect_pred_C, direct_obs, method = "pearson"))^2, 2)
NRMSE = round((sqrt(mean((direct_obs - indirect_pred_C)^2))/(max(direct_obs) - min(direct_obs))) *
  100, 2)
...
beanplot(unlist(PLSPM.r2) - 0.25, rescale(unlist(RF.r2_C), 0, 0.8) + 0.3, col = list("black",
  "gray"), border = NA, innerboerder = NA, beanlines = "median", ll = 0, side = "b",
  ylab = "r²", cex.lab = 1.3, cex.axis = 1.3, las = 1, ylim = c(0, 1))
...
beanplot(unlist(PLSPM.bias), unlist(RF.bias_C), col = list("black", "gray"), border = NA,
  innerboerder = NA, beanlines = "median", ll = 0, side = "b", ylab = "bias", cex.lab = 1.3,
  cex.axis = 1.3, las = 1)
...
GISTools::map.scale(xc=610460,yc=5362570,len=100,units="km", ndivs=1,
                    subdiv=0.1, scol = "black", sfcol =c("black"))
# BM, Rich, Soil
nicergb(RGB.maps, r=2, g=3, b=4, stretch = 20)
plot(limit, lwd=3, lty=2,col="black", add=T)
north.arrow(xb=610430,yb=5362950,len=15,lab="North")
GISTools::map.scale(xc=610460,yc=5362570,len=100,units="km", ndivs=1, subdiv=0.1,
                    scol = "black", sfcol =c("black"))
dev.off()
...
xdat <- data.frame(H = scores[, 1], BM = scores[, 3], Rich = scores[, 4], Depth = scores[,
  5])
...
# plot species ordination par(mai=c(1,0.5,1,0.5))
fig <- plot(nmds1, type = "none", xlab = "Axis 1", ylab = "Axis 2", bty = "l", xaxs = "i",
  yaxs = "i", ylim = c(-1.5, 1), xlim = c(min(NMDS.sp1), max(NMDS.sp1)), cex.lab = 1.5,
  las = 1, cex.axis = 1.5)
points(fig, "sites", pch = 16, cex = data$Carbono_total * 0.18, col = rgb(0, 0, 0,
  60, maxColorValue = 255))
plot(fit1, col = "red", lty = 2, cex = 1.5)
# plot PFTs ordination par(mai=c(1,0.5,1,0.5))
fig <- plot(nmds1, type = "none", xlab = "Axis 1", ylab = "Axis 2", bty = "l", xaxs = "i",
  yaxs = "i", ylim = c(-1.5, 1), xlim = c(min(NMDS.sp1), max(NMDS.sp1)), cex.lab = 1.5,
  las = 1, cex.axis = 1.5)
...
plot(fit1, col = "red", lty = 2, cex = 1.5)
legend("bottomright", legend = c("Conservation", "Productive"), pch = c(1, 2), cex = 1.3,
  bty = "n")
dev.off()
...
plot_ordiAxis <- function(axis, color) {
  ...
  plot(c(min(axis), max(axis)), c(-0.5, 1), type = "n", axes = F, xlab = "", ylab = "",
    main = "")
  ...
  text(x = round(seq(min(axis), max(axis), l = 5), 1), y = -0.2, labels = round(seq(min(axis),
    max(axis), l = 5), 1), cex = 2)
}
